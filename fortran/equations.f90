    ...
    if (.not. EV%is_cosmological_constant) then
        call State%CP%DarkEnergy%PerturbedStressEnergy(dgrho_de, dgq_de, dgpi_de, & !KZ
    ...
    if (.not. EV%is_cosmological_constant) &
        ...
        EV%w_ix, a, adotoa, k, z, ay, dgrho, dgrho_de, dgpi_de, grho, grhov_t) !KZ
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        dgpi  = dgpi_de + grhor_t*pir + grhog_t*pig
        ...
        gpres = gpres_noDE + w_dark_energy_t*grhov_t
        diff_rhopi = pidot_sum - (4*(dgpi-dgpi_de)+ dgpi_diff)*adotoa + & !KZ: take pi_de out, very important for PPF with astress
            State%CP%DarkEnergy%diff_rhopi_Add_Term(dgrho_de, dgq_de, grho, &
            gpres, w_dark_energy_t, State%grhok, adotoa, &
            EV%kf(1), k, grhov_t, z, k2, ayprime, ay, EV%w_ix, dgpi_de) !KZ
